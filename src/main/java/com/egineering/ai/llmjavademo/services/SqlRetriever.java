...
//                .collect(Collectors.joining("\n"));
        ...
        String sql = sqlAgent.generate(DDL, text);

        List<Map<String, Object>> results;
        try {
            results = jdbcTemplate.queryForList(sql); // SQL injection!
        } catch (Exception e) {
            sql = sqlAgent.generate(SqlRetriever.DDL, String.format("I received this error: \"%s\" after originally asking with this message: \"%s\". Can you try again, please?\n\n", e.getCause().getMessage(), text));
            try {
                results = jdbcTemplate.queryForList(sql); // SQL injection!
            } catch (Exception e2) {
                sql = sqlAgent.generate(SqlRetriever.DDL, String.format("I received this error: \"%s\" after originally asking with this message: \"%s\". Can you try again, please?\n\n", e2.getCause().getMessage(), text));
                try {
                    results = jdbcTemplate.queryForList(sql); // SQL injection!
                } catch (Exception e3) {
                    sql = sqlAgent.generate(SqlRetriever.DDL, String.format("I received this error: \"%s\" after originally asking with this message: \"%s\". Can you try again, please?\n\n", e2.getCause().getMessage(), text));
                    try {
                        ...
                        log.error("Error running generated SQL {}. {}", sql, e4.getCause().getMessage());
